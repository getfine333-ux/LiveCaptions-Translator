...
namespace LiveCaptionsTranslator.utils
{
    public static class TextUtil
    {
        ...
        // Language-aware length measurement.
        // For CJK text (no spaces), count characters; for Latin text, count words.
        ...
        // Supports "${VAR_NAME}" syntax so keys never have to live in config files.
        // Checks process env first, then User/Machine scope (registry) so that
        // freshly-set variables work without restarting the shell.
        public static string ResolveSecret(string? value)
        {
            if (string.IsNullOrWhiteSpace(value))
                return string.Empty;

            var match = System.Text.RegularExpressions.Regex.Match(value.Trim(), @"^\$\{([A-Za-z_][A-Za-z0-9_]*)\}$");
            if (!match.Success)
                return value;
